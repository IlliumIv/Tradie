...
namespace Tradie
{
    public class TradieCore : BaseSettingsPlugin<TradieSettings>
    {
        ...
        private Element GetTradingWindow()
        {
            try
            {
                return GameController.Game.IngameState.UIRoot.Children[1].Children[81].Children[3].Children[1].Children[0].Children[0];
            }
            catch (Exception e)
            {
                if (Settings.Debug)
        ...
        private Element GetNpcTadeWindow()
        {
            try
            {
                return GameController.Game.IngameState.UIRoot.Children[1].Children[80].Children[3];
            }
            catch(Exception e)
            {
                if (Settings.Debug)
